...
class AdditionalLayerActor(Actor):
    ...
    def forward_with_noise(self, obs, noise):
        features = self.extract_features(obs)
        output = th.clip(self.first_layers(features) + noise, min=-1, max=1)
        output = self.additional_layer(output, features, None)
        return output
