...
@dataclass
class RoundedGaussianDataLoader(BaseDataLoader):
    scale: float = 1

    def __iter__(self):
        data_loader = GaussianDataLoader(self.batch_size, self.batch_count, self.dim, self.device, self.seed)
        for v in data_loader:
            yield th.round(v*self.scale)
