...
from core.constraints.binary_constraint import StraightThroughRound, Lambda, ConstantLayer
...
class RealNvp(VolumeAwareModule, th.nn.Module, LoadbleModule):
    ...
    def __init__(self, var_count, transform_count, conditional_param_count=0, hidden_size=256, dtype="float64", is_integer=False, s_scaling_factor=1):
        ...
        self.kwargs = {'var_count': var_count, 'transform_count': transform_count, "conditional_param_count": conditional_param_count, "hidden_size": hidden_size, "dtype": dtype, "s_scaling_factor":s_scaling_factor }
        self.mask = nn.Parameter(self._get_masks(transform_count), requires_grad=False)
        self.is_integer = is_integer  
        self.t = self._get_t()
        self.s_scaling_factor = s_scaling_factor # Scaling is limited by tanh. because s is with -1, 1. We scale more to get better edges
    ...
    def f(self, x: th.Tensor) -> Tuple[th.Tensor, th.Tensor]:
        ...
        for i in reversed(range(len(self.t))):
            ...
            if self.is_integer:
                ...
                s = self.s[i](th.cat([z_, y], dim=1)) * (1-self.mask[i])*self.s_scaling_factor
    ...
    def g(self, z):
        ...
        for i in range(len(self.t)):
            ...
            if self.is_integer:
                ...
                s = self.s[i](th.cat([x_, y], dim=1))*(1 - self.mask[i])*self.s_scaling_factor
    ...
    def _get_nett(self):
        layers = [nn.Linear(self.dim, self.hidden_size, dtype=self.dtype),
            nn.LeakyReLU(),
            nn.Linear(self.hidden_size, self.hidden_size, dtype=self.dtype),
            nn.LeakyReLU(),
            nn.Linear(self.hidden_size, self.var_count, dtype=self.dtype)]
        if self.is_integer:
            layers.append(Lambda(StraightThroughRound.apply))
